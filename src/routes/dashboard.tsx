...
function AdminHome() {
  ...
  const fetchEnrollments = useCallback(async () => {
    ...
    try {
      ...
      if (filterActivity !== "all") rows = rows.filter((r) => hasActivity(r, filterActivity));
  ...
  useEffect(() => {
    if (role === "admin") fetchEnrollments();
  ...
  async function handleExport() {
    ...
    try {
      ...
      if (filterActivity !== "all") {
        rows = rows.filter((r) =>
          String(r.Activities).includes(`"activity_name":"${filterActivity}"`),
        );
      }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        ...
        <Card>
          <CardContent className="p-6">
            <div className="text-xs text-muted-foreground">My forms filled</div>
            <div className="mt-1 text-2xl font-semibold">
              {adminProfile?.forms_filled_count ?? 0}
            </div>
            <div className="mt-1 text-xs text-muted-foreground">
              Recent Enrollments below shows submitted forms from all admins.
            </div>
          </CardContent>
        </Card>
      </div>
        ...
        <div className="px-4 pb-4 border-b">
          ...
          {hasFilters && (
            <div className="flex flex-wrap gap-1.5 mt-2">
              ...
              {filterPayment !== "all" && (
                <Chip label={`Payment: ${filterPayment}`} onRemove={() => setFilterPayment("all")} />
              )}
              {filterShift !== "all" && (
                <Chip label={`Shift: ${filterShift}`} onRemove={() => setFilterShift("all")} />
              )}
              {filterGender !== "all" && (
                <Chip label={`Gender: ${filterGender}`} onRemove={() => setFilterGender("all")} />
              )}
              {filterAge !== "all" && (
